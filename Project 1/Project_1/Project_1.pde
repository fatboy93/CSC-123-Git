...
{    
  ...
  if (moving_man)
  {
    if (back)
    {
      x-=random(0, 0);  //make him move backward if back is true;
      if (x > 10)
    ...
    } else
    {
      ...
      if (x<=-375)      // when the man move to the edge on the left 
  ...
  {
    cx = random(width);
    line(cx, height-20, cx+25, height-150);
    cx = random(width);
    line(cx, height, cx+75, height-150);
  }
  noStroke();
...
{
  moving_man = true;
  batman = true;// click to make the man move
}
...
void rightboxer() {
  ...
  //  save("championpose1.jpg");
}

void umbrellaMan() {
  pushMatrix();
  translate(x, y);  //important to make body move forward
  translate(800, 275);  //origin at 100,100
  scale(scale_factor);
...
void middleText()
{
  ...
  fill(255);
  ellipse(413, y3-13, 155, 55);
  triangle(413, y3+24, 423, y3+5, 403, y3+5);
  fill (tb);
  textSize(32);
  text("Help Me", 350, y3);
}